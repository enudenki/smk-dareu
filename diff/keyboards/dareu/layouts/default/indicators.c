...
#define LED_ALL_ROWS LED_ROW_PINS_USER_DEF
...
static void led_regen_one()
{
    led_fb[regen_row][regen_col] = led_brightness;

    uint8_t phase;
    phase = led_phase + (uint8_t)((regen_col * 128) / 21);

    /* !!! WARNING !!! */
    // Seems to work
    // led_fb[regen_row][regen_col] =
    //  (uint8_t)(((uint16_t)sin_table[phase & 0x7F] * led_brightness) / 255);

    // BRICKS THE DEVICE
    // 16-bit math operations can be dangerous here on an 8-bit MCU.
    // led_fb[regen_row][regen_col] =
    //  (uint8_t)(((uint16_t)sin_table[phase & 0x7F] * led_brightness) >> 8);

    // Does not work
    // led_fb[regen_row][regen_col] =
    // ((uint16_t)sin_table[phase & 0x7F] * led_brightness) >> 8;
...
static void led_set_columns()
{
    ...
    SET_PWM_DUTY_2(LED_PWM_C16, LED_DUTY(fb[16]));
    SET_PWM_DUTY_2(LED_PWM_C17, LED_DUTY(fb[17]));
    SET_PWM_DUTY_2(LED_PWM_C18, LED_DUTY(fb[18]));
    SET_PWM_DUTY_2(LED_PWM_C19, LED_DUTY(fb[19]));
    SET_PWM_DUTY_2(LED_PWM_C20, LED_DUTY(fb[20]));
}
